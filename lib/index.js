...
const signMethods = {
    [SignMethod.HMAC_SHA256]: (message, key = '') => Base64.stringify(hmacSHA256(message, key)),
    [SignMethod.MD5]: (message) => md5(message).toString()
};
...
const pathReplace = (string) => string.replace(/\./g, '/');
...
class KsMerchantClient {
    ...
    generateSign(appkey, signSecret, method, signMethod, timestamp, systemParams, paramString) {
        ...
        if (!isFunction$1(signMethodFunction))
            throw Error(`signMethod [${signMethod}] is not support`);
        return signMethods[signMethod](data, signSecret);
    }
    execute(api, method, systemParams, params) {
        ...
        let _data = {};
        let _params = {};
        const baseParams = Object.assign(Object.assign({}, systemParams), { signMethod: this.signMethod, timestamp, sign, method: api });
        if (method === 'POST') {
            _data = Object.assign({ param: sortedParams }, baseParams);
        }
        else {
            _params = Object.assign({ param: paramsString }, baseParams);
        }
        return request({ url: requestUrl, method: method, params: Object.assign({ appkey: this.appKey }, _params), data: _data });
    }
}
...
module.exports = KsMerchantClient;
